...
/* C <- A * B */
static void mulmat(double * a, double * b, double * c, int arows, int acols,
		int bcols) {
	int i, j, l;
...
Point trilateration(Point Point1, Point Point2, Point Point3, double r1,
		double r2, double r3) {
	Point position;

	double p2p1Distance = powf(
			powf(Point2.x - Point1.x, 2) + powf(Point2.y - Point1.y, 2), 0.5);
	Point ex = { (Point2.x - Point1.x) / p2p1Distance, (Point2.y - Point1.y)
			/ p2p1Distance };
	Point aux = { Point3.x - Point1.x, Point3.y - Point1.y };

	double i = ex.x * aux.x + ex.y * aux.y;

	Point aux2 = { Point3.x - Point1.x - i * ex.x, Point3.y - Point1.y
			- i * ex.y };
	Point ey = { aux2.x / norm(aux2), aux2.y / norm(aux2) };

	double j = ey.x * aux.x + ey.y * aux.y;

	double x = (powf(r1, 2) - powf(r2, 2) + powf(p2p1Distance, 2))
			/ (2 * p2p1Distance);
	double y = (powf(r1, 2) - powf(r3, 2) + powf(i, 2) + powf(j, 2)) / (2 * j)
			- i * x / j;
...
static int conexao(int origem, int sinal, Point pos, int *n, int *s, Point *p,
		int limiar, int t, uint8_t hops) {
	int i;
	for (i = 0; i < t; i++) {
